...
export function matchesProduct(product: Product, query: string) {
  const text = normalizeSearch([product.name, product.model, product.brand, product.category, product.subcategory, product.summary, ...product.applications, ...(product.tags || [])].join(' '))
