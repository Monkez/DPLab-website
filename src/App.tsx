...
export function App() {
  ...
  useEffect(() => { const favicon = document.querySelector<HTMLLinkElement>('link[rel="icon"]') ?? document.createElement('link'); favicon.rel = 'icon'; favicon.href = settings.faviconSrc; favicon.removeAttribute('type'); if (!favicon.parentElement) document.head.appendChild(favicon) }, [settings.faviconSrc])
  useEffect(() => { document.documentElement.style.setProperty('--blue', settings.appearance.primaryColor); document.documentElement.style.setProperty('--cyan', settings.appearance.secondaryColor); document.documentElement.style.setProperty('--products-per-row', String(settings.appearance.productsPerRow)); document.body.dataset.cardStyle = settings.appearance.cardStyle }, [settings.appearance])
