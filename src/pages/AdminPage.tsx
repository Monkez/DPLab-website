import { FileText, FolderTree, LayoutDashboard, LogOut, Newspaper, Package, Palette, UserCog, Users } from 'lucide-react'
...
type AdminTab = 'quotes' | 'products' | 'articles' | 'categories' | 'branding' | 'content' | 'display' | 'accounts'
const titles: Record<AdminTab, string> = { quotes: 'Yêu cầu báo giá', products: 'Danh mục sản phẩm', articles: 'Tin tức & bài viết', categories: 'Ngành hàng & phân loại', branding: 'Thương hiệu & liên hệ', content: 'Nội dung website', display: 'Giao diện & hiển thị', accounts: 'Tài khoản & phân quyền' }
...
export function AdminPage({ navigate }: { navigate: (path: string) => void }) {
  ...
  useEffect(() => { if (sessionToken) api.getAdminProfile().then(user => { const next = { token: sessionToken, user }; api.saveAdminSession(next); setSession(next) }).catch(reason => { if (!api.getAdminSession()) setSession(null); else setError(reason instanceof Error ? reason.message : 'Không xác minh được phiên đăng nhập') }) }, [sessionToken])
