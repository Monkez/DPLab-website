  ...
  return <div className="accounts-panel">
    ...
    {editing && <div className="admin-editor-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setEditing(null) }}><form className="admin-editor account-editor" onSubmit={save}>
      ...
      <div className="admin-editor__body">
        ...
        <Field label={editingUsername ? 'Mật khẩu mới (để trống nếu giữ nguyên)' : 'Mật khẩu'} wide><div className="password-field"><KeyRound /><input type="password" value={editing.password} onChange={event => setEditing({ ...editing, password: event.target.value })} required={!editingUsername} autoComplete="new-password" /></div></Field>
