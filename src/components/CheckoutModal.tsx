...
export function CheckoutModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <div className="modal-shell" role="dialog" aria-modal="true" aria-label="Đặt hàng">
    ...
    <div className="checkout-modal">
      ...
      {submitted ? <div className="success-view"><span className="success-icon"><CheckCircle2 size={42} /></span><p className="eyebrow">ĐẶT HÀNG THÀNH CÔNG</p><h2>Cảm ơn bạn đã chọn DTPT Shop!</h2><p>Đơn <strong>#{submitted.id}</strong> đã được ghi nhận. Nhân viên sẽ gọi xác nhận trong giờ làm việc.</p><div className="success-order"><span>Tổng giá trị đơn hàng</span><strong>{money(submitted.total)}</strong></div><a className="button button--primary button--wide" href={`tel:${settings.phone.replace(/\s/g, '')}`}><Phone size={18} />Gọi ngay {settings.phone}</a><button className="text-button" onClick={close}>Tiếp tục mua sắm <ChevronRight size={16} /></button></div> : <div className="checkout-grid">
        <form onSubmit={submit} className="checkout-form"><p className="eyebrow">THÔNG TIN NHẬN HÀNG</p><h2>Hoàn tất đặt hàng</h2><div className="form-grid"><label>Họ và tên *<input name="name" autoFocus placeholder="Nguyễn Văn A" /></label><label>Số điện thoại *<input name="phone" inputMode="tel" placeholder="09xx xxx xxx" /></label><label className="form-span">Địa chỉ nhận hàng *<input name="address" placeholder="Số nhà, đường, quận/huyện..." /></label><label className="form-span">Ghi chú<textarea name="note" rows={3} placeholder="Thời gian nhận hàng, yêu cầu thêm..." /></label></div>{error && <p className="form-error">{error}</p>}<div className="offline-note"><ShieldCheck size={19} /><span>Không cần thanh toán online. DTPT Shop sẽ gọi xác nhận trước khi giao.</span></div><button className="button button--primary button--wide" type="submit">Xác nhận đặt hàng · {money(cartTotal)}</button></form>
