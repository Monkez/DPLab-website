...
function pageSeo(pathname, data) {
  ...
  const title = pathname === '/huong-dan-mua-hang' ? `Thông tin doanh nghiệp & hướng dẫn mua hàng | ${name}` : article ? article.seoTitle || `${article.title} | ${name}` : product ? product.seoTitle || `${product.name} ${product.model} | ${name}` : pathname === '/tin-tuc' ? `Tin tức công nghệ công nghiệp | ${name}` : pathname === '/san-pham' ? `Thiết bị công nghiệp | ${name}` : `${name} | ${settings.slogan || 'Thiết bị đo lường và tự động hóa'}`
