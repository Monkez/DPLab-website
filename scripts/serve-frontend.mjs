...
function pageSeo(pathname, data) {
  ...
  const title = pathname === '/huong-dan-mua-hang' ? `Thông tin doanh nghiệp & hướng dẫn mua hàng | ${name}` : article ? article.seoTitle || `${article.title} | ${name}` : product ? product.seoTitle || `${product.name} ${product.model} | ${name}` : pathname === '/tin-tuc' ? `Tin tức công nghệ công nghiệp | ${name}` : pathname === '/san-pham' ? `Thiết bị công nghiệp | ${name}` : `${name} | ${settings.slogan || 'Công nghệ tiên tiến. Giá trị bền vững.'}`
  ...
  const schema = article ? { '@context': 'https://schema.org', '@type': 'Article', headline: article.title, description, image: [image], datePublished: article.publishedAt, dateModified: article.updatedAt, author: { '@type': 'Organization', name: article.author }, publisher: { '@type': 'Organization', name }, mainEntityOfPage: canonical } : product ? { '@context': 'https://schema.org', '@type': 'Product', name: `${product.name} ${product.model}`, description, image: [image], brand: { '@type': 'Brand', name: product.brand }, model: product.model, sku: product.id, offers: product.price && product.priceMode === 'fixed' ? { '@type': 'Offer', priceCurrency: 'VND', price: product.price, availability: 'https://schema.org/PreOrder', url: canonical } : undefined } : { '@context': 'https://schema.org', '@type': 'Organization', name, url: siteUrl, logo: image, email: settings.email, telephone: settings.phone }
...
const server = http.createServer(async (request, response) => {
  ...
  const isPage = request.method === 'GET' && (pathname === '/' || pathname === '/huong-dan-mua-hang' || pathname === '/san-pham' || pathname.startsWith('/san-pham/') || pathname === '/tin-tuc' || pathname.startsWith('/tin-tuc/') || pathname.startsWith('/admin'))
  if (isPage) {
    try {
